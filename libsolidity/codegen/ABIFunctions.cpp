...
)
{
	...
	return createFunction(functionName, [&]() {
		...
		Whiskers templ(
			...
			R"(
				...
				function <functionName>(value, pos) <return> {
					...
					for { let i := 0 } lt(i, length) { i := add(i, 1) }
					{
						<assignToPos> <encodeToMemoryFun>(<arrayElementAccess>, pos)
						srcPtr := <nextArrayElement>(srcPtr)
						<incrementPos>
					}
					<assignEnd>
		...
		if (!usesTail)
		{
			if (_options.dynamicInplace && dynamicBase)
			{
				templ("assignToPos", "pos :=");
				templ("incrementPos", "");
			}
			else
			{
				templ("assignToPos", "");
				// Always use padded size because array elements are always padded.
				string elementEncodedSize = toCompactHexWithPrefix(_to.baseType()->calldataEncodedSize());
				templ("incrementPos", "pos := add(pos, " + elementEncodedSize + ")");
			}
		}
...
)
{
	...
	return createFunction(functionName, [&]() {
		...
		else
		{
			...
			// more than desired, i.e. it alwayswrites beyond the end of memory.
			Whiskers templ(
				R"(
					// <readableTypeNameFrom> -> <readableTypeNameTo>
					function <functionName>(value, pos) <return> {
